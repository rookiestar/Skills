...
extracts all fields (POS, phonetics, definitions, translations, examples, CEFR),
...
def extract_entry(html: str, word: str, url: str) -> CambridgeEntry | None:
    ...
    # POS
    pos_node = entry.select_one(".pos, .dpos")
    if pos_node:
        pos_text = pos_node.get_text(strip=True).lower()
        if pos_text in CAMBRIDGE_TO_ECDICT:
            result.pos = CAMBRIDGE_TO_ECDICT[pos_text]
    ...
    # Senses: def-block iteration
    blocks = soup.select("div.def-block")
    if not blocks:
        ...
        if ex_node:
            result.example = clean_text(ex_node.get_text(" ", strip=True))
    ...
    for block in blocks[:5]:
        ...
        if def_text and trans_text:
            ...
            if combined not in result.definitions:
                result.definitions.append(combined)
...
def already_fetched(conn: sqlite3.Connection, word: str) -> bool:
    row = conn.execute(
        "SELECT 1 FROM dictionary WHERE word = ? AND source = 'cambridge' AND COALESCE(TRIM(pos), '') != ''",
        (word,),
    ).fetchone()
...
def write_entry(conn: sqlite3.Connection, entry: CambridgeEntry) -> None:
    ...
    conn.execute(
        ...
        INSERT OR REPLACE INTO dictionary
            (word, phonetic, phonetic_uk, phonetic_us, pos, definition, translation,
    ...
    for rank, term in enumerate(reverse_terms[:5]):
        conn.execute(
            """INSERT OR IGNORE INTO dictionary_reverse
               (zh_term, word, pos, phonetic, source, sense_rank)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (term, entry.word, entry.pos, entry.phonetic_us or entry.phonetic_uk, "cambridge", rank),
        )
