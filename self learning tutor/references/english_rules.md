...
- Default to the local dictionary data as the runtime source.
- Runtime sources:
  - `data/dictionary.db`
  - `data/sample_dictionary.json`
- The lookup flow should use `scripts/dict_lookup.py`; do not use Cambridge at runtime.
- Cambridge is a build-time curation reference only, not a live lookup step.
- Every factual part of the answer must come from the matching local entry or a direct paraphrase of it.
- Do not invent meanings, examples, usage notes, or extra synonyms.
- If the local data does not clearly support a field, omit that field rather than guessing.
...
- Show 2 meanings by default when the local data supports them.
- Show a 3rd meaning only on follow-up and only if the local data clearly supports it.
...
- If the local data clearly supports only 1 meaning, show 1 and stop.
- Keep examples short and natural.
- The main job is to explain the English word in Chinese.
- Use only local-dictionary-provided example sentences; do not create a new example sentence.
- Do not explain etymology, grammar theory, or exam strategy.
- If the local data does not provide an example sentence, omit the example line.
- Do not add a 3rd meaning in the initial reply.
- If the user asks for more meanings, add the 3rd only if the local data clearly supports it.
...
- Give the 2 most common English equivalents when the local data clearly supports them.
- The main job is to give the English answer first, then help the student confirm it.
- Use only local-dictionary-provided example sentences; do not create a new example sentence.
- Do not give a long list of candidates.
- If the local data does not provide a safe example sentence, omit the example line.
- Do not add a 3rd English equivalent in the initial reply.
- If the user asks for more, add the 3rd only if the local data clearly supports it.
- Do not add any line that is not shown in the template.
...
- If the follow-up needs information that the local data does not clearly support, say you need the word again or keep the answer minimal.
- Do not end a normal lookup with "want to know more?" or another open-ended invitation.
- Do not write or update any memory file, including `MEMORY.md`. Do not ask the runtime to persist conversation state.
